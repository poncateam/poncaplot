...
struct DrawingPass {
    virtual void render(const DataManager::PointCollection& points, uint8_t*buffer, int w, int h) = 0;
...
struct DisplayPoint : public DrawingPass {
    ...
    void render(const DataManager::PointCollection& points, uint8_t*buffer, int w, int h) override{
        using VectorType = typename DataManager::PointCollection::VectorType;
